...
clean_counts <- function(in_counts_file, 
  ...
  columns_with_pos <- colnames(pos_rows)[
    apply(pos_rows, 2, function(x) max(x) >= minimum_count)]
  clean_df <- clean_df[, !(names(clean_df) %in% columns_with_pos)]
  
  neg_rows <- clean_df[clean_df$Sample.Number == "NEG", 
                       !(names(clean_df) %in% 
                           c(names(features_df), sep_col_name))]
  columns_with_neg <- colnames(neg_rows)[
    apply(neg_rows, 2, function(x) max(x) >= minimum_count)]
